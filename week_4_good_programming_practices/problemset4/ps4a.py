...
def isValidWord(word, hand, wordList):
    ...
    handCopy = hand.copy()
    for ch in word:
        if ch in handCopy.keys() and handCopy[ch] != 0:
            i += 1
            handCopy[ch] -= 1
    if len(word) == i and word in wordList:
        return True
    else:
        return False
...
def playHand(hand, wordList, n):
    ...
    * The hand finishes when there are no more unused letters or the user
    ...
    #             total_score += getWordScore(word, len(hand))
    #             print('"' + word + '"' + ' earned ' + str(getWordScore(word,len(hand))), end = " ")
    ...
    while sum(hand.values()) != 0:
        ...
        displayHand(hand)
        print('Enter a word, or a "." to indicate that you are finished: ', end = " ")
        word = input()
        if word == '.':
            break
        elif word != '.':
